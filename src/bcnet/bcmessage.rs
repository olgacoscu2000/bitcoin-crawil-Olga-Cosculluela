...
lazy_static! {
    ...
    pub static ref MEMPOOL:String = String::from("mempool");//request for info about transactions
    pub static ref SEND_CMPCT:String = String::from("sendcmpct");//mirar nodos duplicados
...
pub fn read_message(mut connection: &TcpStream) -> Result<(String, Vec<u8>), Error> {
    ...
    //no elige la información que necesita, el va leyendo todo el rato y en otra función le dice que 
    //si lo que ha recibido no se equivale con lo que quiere lo tira y lee el siguiente.

    //si está ok es que es la respuesta que él quiere, sabemos qué hay y sabemos cómo se organiza
    println!("messages 2");
    let mut header_buffer = [0 as u8;HEADER_SIZE];

    return match connection.read(&mut header_buffer) {

        //primero leemos tanto bytes como sabemos que tiene el header, despues averiguamos
        //los bytes que de verdad son útiles y lugo almaenamos solo esos. 

        //CÓMO SABE CUÁLES SON LOS QUE LE INTERESAN?? QUÉ HAY DESPUÉS DE ESO?? 
...
//a partir de un bloque, pedir algunas transacciones de las del bloque
fn build_transactions_request(block_id: & Vec<u8>, indexes_length: & Vec<u8>, indexes: & Vec<u8>){
...
pub fn process_addr_message(payload: &Vec<u8>) -> Vec<String>{
    ...
    while read_addr < addr_number {
        ...
        //protocolo bitcoin-> sabe cómo son, las procesa así
        ...
        //eprintln!("--> Ajout {} noeuds", new_peer);
