...
#[derive(Debug)] //identificador más referencia al de antes
...
lazy_static! {
    ...
    //mutex=mutual exlcusion, solo uno de esos valores al mismo tiempo
}
...
pub fn create_block_message_payload(blocks_id: &Vec<(String, bool)>) {
    ...
    //un mensaje bitconi tiene siempre la misma estructura, y hay una parte que no cambia (a excepción de la date-> por
    // eso tenemos una función para cambiarla)
    //el payload es la parte que sí que cambia.
