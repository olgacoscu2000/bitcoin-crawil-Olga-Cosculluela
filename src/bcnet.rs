...
use bcmessage::{INV, MSG_VERSION, MSG_VERSION_ACK, MSG_GETADDR, CONN_CLOSE, MSG_ADDR, GET_HEADERS, HEADERS, GET_BLOCKS, BLOCK, GET_DATA, GET_BLOCK_TXN, BLOCK_TXN };
...
pub fn handle_one_peer(connection_start_channel: Receiver<String>, address_channel_tx: Sender<String>, _num: u64){
    ...
    loop{ //Node Management
        ...
        match TcpStream::connect_timeout(&socket, CONNECTION_TIMEOUT) {
            //aqui es donde le damos "nombre" al canal
        ...
        match bcmessage::read_message(&connection) {
            ...
            Ok((command, payload)) => {
                ...
                match command {
                    ...
                    cmd if cmd == *BLOCK_TXN 
                        => return match handle_incoming_cmd_msg_transaction() {
                            true  => &GET_BLOCK_TXN,
                            false => &CONN_CLOSE
                        },
...
fn handle_incoming_cmd_msg_transaction(){
